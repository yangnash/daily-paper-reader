...
tldr: 针对组合视频检索中查询由源视频和文本编辑指令共同定义、传统方法难以表达目标变化的问题，提出零样本R^3管道。首先生成推理轨迹描述编辑后的目标视频，再将轨迹与源视频编码为推理增强查询，通过一致性门控残差融合分数。最后用重排序器直接比较源-候选对验证候选。实验证明该方法有效，代码已开源。
source: arxiv
...
motivation: 组合视频检索中嵌入模型难以表达目标状态变化、动作替换等细节，而遍历全库重排计算不可行。
method: 将源-编辑查询转化为推理引导的检索程序：生成推理轨迹作为推理增强查询，与基础查询分数经一致性门控残差融合，最后用重排序器验证候选。
result: 在CoVR-R挑战中该方法有效提升了零样本组合视频检索的性能。
conclusion: R^3通过推理引导的召回和重排序，在不需训练的情况下实现了对组合视频检索的准确和高效处理。
---

## 摘要
CoVR-R挑战评测组合视频检索，要求系统根据参考视频和文本编辑指令从大型图库中检索目标视频。此设定不是标准的视频-文本检索问题：查询由源视频中的视觉证据和编辑隐含的变换共同定义。强嵌入模型可提供可扩展的候选召回，但可能无法充分表达目标侧结果，如状态变化、动作替换、对象保持或时间一致性。成对多模态重排序器能更直接地验证此类细节，但对整个图库进行全面重排序在计算上不可行。我们提出R^3，一种围绕推理引导的召回与重排序构建的零样本组合视频检索流水线。核心思路是将源-编辑查询转化为推理基础的检索程序，而非将编辑文本视为短标题。首先，模型生成推理轨迹，描述应用编辑后的预期目标视频。然后，将轨迹与源视频一起编码为推理增强查询，并通过一致性门控残差规则与基础组合查询的检索得分融合。最后，重排序器通过直接的源-候选比较验证召回的候选。实验证明了我们方法在应对该挑战中的有效性。代码可在https://github.com/Lee-zixu/R-3获取。
